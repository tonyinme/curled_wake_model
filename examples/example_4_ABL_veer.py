...
def main(args):
    ...
    for theta in theta_ls:
        ...
        wf = cwm.wind_farm_class(
                    ...
                    h=90,
                    turbines=turbines,
                    saveDir='example_4_results',
                )

        # Add the boundary layer
        wf.add_boundary_layer(TI=0.1)
        # Add the turbulence model
        wf.add_turbulence_model()
        # Add veer
        wf.add_veer(mode='theta', theta=theta, th=90, D=126)

        # Call the volver
        wf.solve(nut_model='standard')

        wf.plot_streamwise(name=f'veer_{theta}.png')

        # Power of all turbines
        power = [t.power() for t in turbines]

        plt.plot(power, 'o-', label=theta)
